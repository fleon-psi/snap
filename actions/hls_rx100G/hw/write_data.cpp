...
#pragma HLS PIPELINE II=129
			...
			if ((is_head) && (packet_counter_addr % 64 == 0)) {
				// Save old packet counter
				if (packet_counter[packet_counter_addr/64].head != frame_number0) {
					...
					memcpy(dout_gmem + out_frame_status_addr + 1 + (packet_counter[packet_counter_addr/64].head / 64), &tmp, 64);
				}
...
#pragma HLS UNROLL
		...
		memcpy(dout_gmem + out_frame_status_addr + 1 + (packet_counter[i].head / 64), &tmp, 64);
	}
